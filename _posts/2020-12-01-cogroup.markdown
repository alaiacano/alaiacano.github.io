...
def cogroup[KEY, A, B](a: Iterable[(KEY, A)], b: Iterable[(KEY, B)]): Iterable[(KEY, (Iterable[A], Iterable[B]))] = {
  ...
  (aGrouped.keys ++ bGrouped.keys).toSet.map(coGroupByKey)
}
```
...
val cogrouped: Iterable[(String, (Iterable[Double], Iterable[String]))] = cogroup(data1, data2)
...
```scala
Iterable(
  (a,(Iterable(1.0),      Iterable("aye"))),
  (b,(Iterable(2.0, 2.1), Iterable("bee"))),
  (c,(Iterable(),         Iterable("sea"))),
  (d,(Iterable(4.0),      Iterable()))
)
```
...
There's one thing to note in this implmentation of `cogroup` - the use of `groupMap` on each of the inputs (which, as written, is functionally similar to `groupBy` in Spark/Scio/Scalding etc).

We are making an initial pass through all of the input data to build a key -> value(s) mapping for all of our data, so that we can then do fast lookups to build the final result.

In a distributed system, this is what is called the "shuffle" stage. Instead of building a large `Map[KEY, Iterable[A]]` and `Map[KEY, Iterable[B]]` _in memory_, it moves all data with the same `KEY` to a common machine, so that we can then process the `Iterable[A]` and `Iterable[B]` associated with the particular `KEY`. This requires moving lots of data around and is often the most expensive part of a data pipeline. It's worth considering when and how you shuffle your data.
